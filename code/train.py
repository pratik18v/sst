...
def parse_args():
    ...
    p.add_argument('-e', '--num_epochs', default=10,
...
def main(args):
    ...
    w0 = np.mean(((y_train == 1).sum(1).astype(float) / args.seq_length), axis=0)
    w1 = np.mean(((y_train == 0).sum(1).astype(float) / args.seq_length), axis=0)
    ...
    target_var = T.ltensor3('target')
    sst_model = load_model(input_var=input_var, target_var=target_var, args=args, w0=w0, w1=w1)
    sst_model.compile()

    print 'Loading parameters (if exist) ...'
    filename = args.param_dir + 'params_sl{}_np{}_d{}_w{}_fd{}.hkl'.format(args.seq_length, args.num_proposals, args.depth, args.width, args.feat_dim)
    ...
    f = open(args.param_dir + 'configs', 'w')
